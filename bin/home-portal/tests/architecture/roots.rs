...
pub const RULE: &str = "root";
pub const ROOT_FILES: [&str; 2] = ["lib.rs", "mod.rs"];

pub fn violations(files: &[SourceFile], exceptions: &[Exception]) -> Vec<String> {
    files
        .iter()
...
fn module_roots_hold_declarations_only() {
    let found = violations(&rust_files(), &listed());
...
#[test]
fn a_listed_exception_is_not_reported() {
    let sample = SourceFile::sample("crates/x/src/thing/mod.rs", "pub struct Thing;\n");
    let exception = Exception {
        rule: RULE.to_string(),
        path: "crates/x/src/thing/mod.rs".to_string(),
    };
    assert!(violations(&[sample], &[exception]).is_empty());
}
