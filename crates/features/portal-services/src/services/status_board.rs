use std::collections::HashMap;
use std::sync::{Arc, Mutex, PoisonError, RwLock};

use portal_feature::{StatusChange, StatusObserver};
use portal_model::{ProbeOutcome, ServiceStatus};
use time::OffsetDateTime;

use super::ServiceHistory;
use crate::types::{HistoryRange, HistoryView, HistoryWrite, ServiceEntry, Tracked};

pub struct StatusBoard {
    started_at: OffsetDateTime,
    tracked: RwLock<HashMap<String, Tracked>>,
    forgotten: Mutex<HashMap<String, Option<Tracked>>>,
    observers: Vec<Arc<dyn StatusObserver>>,
}

impl StatusBoard {
    pub fn watched(
        started_at: OffsetDateTime,
        observers: Vec<Arc<dyn StatusObserver>>,
    ) -> StatusBoard {
        StatusBoard {
            started_at,
            tracked: RwLock::new(HashMap::new()),
            forgotten: Mutex::new(HashMap::new()),
            observers,
        }
    }

    pub fn restore(&self, histories: HashMap<String, ServiceHistory>) {
        let mut tracked = self.tracked.write().unwrap_or_else(PoisonError::into_inner);
        for (id, history) in histories {
            tracked.insert(
                id,
                Tracked {
                    status: ServiceStatus::unknown(self.started_at),
                    history,
                },
            );
        }
    }

    pub fn status(&self, id: &str) -> ServiceStatus {
        self.tracked
            .read()
            .unwrap_or_else(PoisonError::into_inner)
            .get(id)
            .map(|tracked| tracked.status.clone())
            .unwrap_or_else(|| ServiceStatus::unknown(self.started_at))
    }

    pub fn history(&self, id: &str, range: HistoryRange, now: OffsetDateTime) -> HistoryView {
        let at = now.unix_timestamp();
        self.tracked
            .read()
            .unwrap_or_else(PoisonError::into_inner)
            .get(id)
            .map(|tracked| tracked.history.view(range, at))
            .unwrap_or_else(|| ServiceHistory::default().view(range, at))
    }

    pub fn record(&self, entry: &ServiceEntry, outcome: ProbeOutcome, now: OffsetDateTime) {
        let id = entry.id.as_str();
        let change = {
            let mut tracked = self.tracked.write().unwrap_or_else(PoisonError::into_inner);
            let current = tracked.entry(id.to_string()).or_insert_with(|| Tracked {
                status: ServiceStatus::unknown(self.started_at),
                history: ServiceHistory {
                    rewrite: true,
                    ..ServiceHistory::default()
                },
            });
            current.history.record(now.unix_timestamp(), &outcome);
            let next = current.status.after(outcome, now);
            let change = (next.state != current.status.state).then(|| StatusChange {
                service: id.to_string(),
                name: entry.name.clone(),
                was: current.status.state.name().to_string(),
                now: next.state.name().to_string(),
                error: next.last_error.clone(),
                diagnosis: next.diagnosis.map(|diagnosis| diagnosis.code().to_string()),
                notify: entry.notifies(),
            });
            current.status = next;
            change
        };
        if let Some(change) = change {
            for observer in &self.observers {
                observer.changed(&change);
            }
        }
    }

    pub fn forget(&self, id: &str) {
        let mut tracked = self.tracked.write().unwrap_or_else(PoisonError::into_inner);
        let set_aside = tracked.remove(id);
        let mut forgotten = self
            .forgotten
            .lock()
            .unwrap_or_else(PoisonError::into_inner);
        let earlier = forgotten.remove(id).flatten();
        forgotten.insert(id.to_string(), set_aside.or(earlier));
    }

    pub fn rename(&self, from: &str, to: &str) {
        let mut tracked = self.tracked.write().unwrap_or_else(PoisonError::into_inner);
        let mut forgotten = self
            .forgotten
            .lock()
            .unwrap_or_else(PoisonError::into_inner);
        let set_aside = forgotten.remove(from).flatten();
        if let Some(mut moved) = tracked.remove(from).or(set_aside) {
            moved.history.dirty = true;
            moved.history.rewrite = true;
            tracked.insert(to.to_string(), moved);
        }
        forgotten.insert(from.to_string(), None);
        forgotten.remove(to);
    }

    pub fn take_dirty(&self) -> Vec<(String, HistoryWrite)> {
        let mut tracked = self.tracked.write().unwrap_or_else(PoisonError::into_inner);
        tracked
            .iter_mut()
            .filter(|(_, tracked)| tracked.history.dirty)
            .map(|(id, tracked)| {
                let history = &mut tracked.history;
                history.dirty = false;
                let write = if history.rewrite {
                    history.rewrite = false;
                    history.unwritten.clear();
                    history.touched_hours.clear();
                    HistoryWrite::Rewrite(history.clone())
                } else {
                    HistoryWrite::Append(history.take_lines())
                };
                (id.clone(), write)
            })
            .collect()
    }

    pub fn rewrite_later(&self, id: &str) {
        let mut tracked = self.tracked.write().unwrap_or_else(PoisonError::into_inner);
        if let Some(current) = tracked.get_mut(id) {
            current.history.rewrite = true;
            current.history.dirty = true;
        }
    }

    pub fn take_forgotten(&self) -> Vec<String> {
        let tracked = self.tracked.read().unwrap_or_else(PoisonError::into_inner);
        self.forgotten
            .lock()
            .unwrap_or_else(PoisonError::into_inner)
            .drain()
            .map(|(id, _)| id)
            .filter(|id| !tracked.contains_key(id))
            .collect()
    }
}
