...
const axes = { from: FROM, to: TO, formatTime: (moment: number) => `t${new Date(moment).getUTCMinutes()}`, formatValue: (value: number) => `${value} мс` };
...
it("breaks the line where a probe failed and where no probe ran, never drawing zero", () => {
  ...
  const { container } = render(<LatencyChart samples={samples} title="Задержка" empty="Нет данных" {...axes} />);
  expect(screen.getByRole("img", { name: "Задержка" })).toBeInTheDocument();
...
it("labels a millisecond axis in round steps reaching the highest value", () => {
  ...
  expect(labels).toEqual(["0 мс", "200 мс", "400 мс", "600 мс"]);
...
it("says there is no data instead of drawing an empty axis", () => {
  render(<LatencyChart samples={[]} title="Задержка" empty="Нет данных" {...axes} />);
