...
it("draws one slot per period, colours it by state and leaves a period without probes hollow", () => {
  const { container } = render(
    <UptimeStrip
      ...
      slots={[
        { key: "1", state: "up", label: "10:00 — работает" },
        { key: "2", state: null, label: "11:00 — нет данных" },
        { key: "3", state: "down", label: "12:00 — недоступен" },
      ]}
    />,
  );
  expect(screen.getByRole("img", { name: "Доступность" })).toBeInTheDocument();
  ...
  expect(container.querySelector('[data-state="down"]')).toHaveAttribute("title", "12:00 — недоступен");
});
...
it("marks the same times at the same places as the latency chart of the range", () => {
  ...
  const strip = render(<UptimeStrip title="Доступность" from={FROM} to={TO} formatTime={formatTime} slots={[]} />);
  const chart = render(
    <LatencyChart
      title="Задержка"
      empty="—"
